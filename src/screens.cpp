...
void screens::updateBandScanMode(bool in_setup, uint8_t channel, uint8_t rssi, uint8_t channelName, uint16_t channelFrequency, uint16_t rssi_setup_min_a, uint16_t rssi_setup_max_a) 
{
    ...
    if(!in_setup) 
    {
        if (rssi > RSSI_SEEK_TRESHOLD) 
        {
            if(best_rssi < rssi) 
            {
                ...
                TV.print(32, SCANNER_LIST_Y_POS, (int)channelFrequency);
            }
            else 
            {
    ...
    else 
    {
        ...
        TV.print(50, SCANNER_LIST_Y_POS, (int)rssi_setup_min_a , DEC);

        TV.print(110, SCANNER_LIST_Y_POS, "   ");
        TV.print(110, SCANNER_LIST_Y_POS, (int)rssi_setup_max_a , DEC);
    }
...
{
    uint8_t rssi_scaled=map(rssiA, 1, 100, 1, 100);
    // clear last bar
    TV.draw_rect(25+rssi_scaled, 6+4*MENU_Y_SIZE, 100-rssi_scaled, 8 , BLACK, BLACK);
    //  draw new bar
    TV.draw_rect(25, 6+4*MENU_Y_SIZE, rssi_scaled, 8 , WHITE, (active_receiver==useReceiverA ? WHITE:BLACK));

    // read rssi B
    rssi_scaled=map(rssiB, 1, 100, 1, 100);
    // clear last bar
    TV.draw_rect(25+rssi_scaled, 6+5*MENU_Y_SIZE, 100-rssi_scaled, 8 , BLACK, BLACK);
    //  draw new bar
